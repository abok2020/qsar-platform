...
@app.command("train-hf")
def train_hf(input: str, output: str = "data/oof/hf_oof.parquet") -> None:
    import pandas as pd
    from pathlib import Path
    from qsar_platform.training.train_hf_embed import train_hf_embed_model
    from qsar_platform.utils.mlflow_utils import (
        start_run,
        log_common_params,
        log_metric,
    ...
    with start_run("train-hf"):
        oof, auc, n_splits = train_hf_embed_model(df)

        out_df = df.copy()
        out_df["hf_embed_xgb_oof"] = oof

        Path(output).parent.mkdir(parents=True, exist_ok=True)
        out_df.to_parquet(output, index=False)

        log_common_params("hf_embed_xgb", n_splits, input, output)
        log_metric("roc_auc", auc)
        log_artifact(output)

        print(f"[HF] AUC: {auc:.4f}")
        print(f"Saved to {output}")
...
def ensemble_oof(
    ...
    hf_input: str = "data/oof/hf_oof.parquet",
    ...
    with start_run("ensemble-oof"):
        ...
        mlflow.log_param("base_models", "ecfp_xgb,desc_lgbm,hf_embed_xgb")
        log_metric("roc_auc", auc)
        log_artifact(output)

        print(f"[ENSEMBLE] Logged ROC-AUC: {auc:.4f}")
